...
/**
 * A `PagingSource` implementation for loading paginated popular anime data.
 *
 * @param animeAPI The AnimeAPI instance used to make API calls for retrieving popular anime data.
 * @param type The optional type of anime content to filter by.
 *             If provided, it filters the popular anime data based on the specified type.
 *             Default is null, which means no specific type filter is applied.
 * @param filter The optional filter parameter used for fetching popular anime data based on a specific filter.
 *               Default is null, which means no specific filter is applied.
 * @param rating The optional rating parameter used for fetching popular anime data with a specific rating.
...
class PopularAnimePagingSource(
    ...
    /**
     * Loads the paginated popular anime data based on the provided [params].
     *
